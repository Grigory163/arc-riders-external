    ...
    namespace detail {
        ...
        struct lazy_module : lazy_base<lazy_module<OHP>> {
            ...
            LAZY_IMPORTER_FORCEINLINE static T in( Ldr ldr ) noexcept {
                safe_module_enumerator ee( reinterpret_cast< const detail::win::LDR_DATA_TABLE_ENTRY_T* >( ldr ) );
                do {
                    if ( hash( ee.value->BaseDllName, get_offset( OHP ) ) == get_hash( OHP ) )
                        return ( T )( ee.value->DllBase );
                } while ( ee.next( ) );
                return {};
            }
